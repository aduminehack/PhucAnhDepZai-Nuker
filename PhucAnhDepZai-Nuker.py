# Copyright © Phuc Anh Dep Zai-Nuker
#########################################################
...
try:
    ...
    with open('data/logins.json', 'w') as f:
            print(f"\n[{g}#\x1b[95m\x1B[37m] Logging Into Phuc Anh Dep Zai-Nuker")
...
#   JOINER
    if choice == '1':
        ...
        gh = input(f"""
Joiner is in the Paid Version of Phuc Anh Dep Zai-Nuker!\nIf You Are Wanting to Purchase Make Sure to Checkout the Offical Phuc Anh Dep Zai-Nuker Website!
                                   
[\x1b[95m1\x1b[95m\x1B[37m] Phuc Anh Dep Zai-Nuker Website
[\x1b[95m2\x1b[95m\x1B[37m] Exit
  
[\x1b[95m>\x1b[95m\x1B[37m] Choice?: """)

        if gh in ['01','1']:
            webbrowser.open('https://phucanh.org')
        elif gh in ['02','2']:
            exit = spammer()
        else:
         print('')
...
#   TOKEN ONLINER
    if choice == '4':
            ...
            config = {
                ...
                "state": "https://phucanh.org",
                "name": "Phuc Anh Dep Zai NUKER",
            }
...
#   ACCOUNT NUKER
    if choice == '7':
        ...
        if threading.active_count() < threads:
            threading.Thread(target=utilities.Plugins.Account_Nuker.PHUCANHDEPZAINUKER_START, args=(token, Server_Name, message_Content)).start()
...
#       width = os.get_terminal_size().columns
        ...
        def main():
            ...
            PhucAnhDepZai = commands.Bot(command_prefix=prefix, intents=discord.Intents().all())
            PhucAnhDepZai.remove_command('help')

            @PhucAnhDepZai.event
            async def on_ready():
                if len(PhucAnhDepZai.guilds) > 1:
                    guildpl = "guilds"
                else:
                    guildpl = "guild"
                activity = discord.Game(name=f"Nuker By Phuc Anh Dep Zai", type=3)
                await PhucAnhDepZai.change_presence(status=discord.Status.dnd, activity=activity)
                clear()
                print(f"[\x1b[95m>\x1b[95m\x1B[37m] Bot : {PhucAnhDepZai.user} ({len(PhucAnhDepZai.guilds)} {guildpl})")
                ...
                print(f"")
                print(f"[\x1b[95m>\x1b[95m\x1B[37m] type: {prefix}bay in a channel")
                print(f"")
            ...
            @PhucAnhDepZai.command()
            async def bay(ctx):
                await ctx.message.delete()
                print(f"[\x1b[95m>\x1b[95m\x1B[37m] Nuking {ctx.guild.name} ({ctx.guild.id})...")
                await ctx.guild.edit(name="Phuc Anh Dep Zai Runs Discord")
...
#   DM SPAMMER
    if choice == '10':
        ...
        gh = input(f"""
DM Spammer is in the Paid Version of Phuc Anh Dep Zai-Nuker!\nIf You Are Wanting to Purchase Make Sure to Checkout the Offical Phuc Anh Dep Zai-Nuker Website!
...
#   WEBHOOK SPAMMER
    if choice == '15':
        ...
        def PhucAnhDepZai():
            ...
            while True:
                for i in range(15):
                    threading.Thread(target=PhucAnhDepZai).start()
        PhucAnhDepZai()
...
#   TOKEN GENERATOR
    if choice == '25':
        ...
        gh = input(f"""
Token Generator is in the Paid Version of PhucAnhDepZai-Nuker!\nIf You Are Wanting to Purchase Make Sure to Checkout the Offical PhucAnhDepZai-Nuker Website!
                                   
[\x1b[95m1\x1b[95m\x1B[37m] PhucAnhDepZai-Nuker Website
[\x1b[95m2\x1b[95m\x1B[37m] Exit
...
#   EXIT
    if choice == '32':
        Spinner()
        exit = True if input(f"\n[{Fore.LIGHTMAGENTA_EX}>{Fore.RESET}] Are You Sure You Want To Exit Phuc Anh Dep Zai-Nuker? Y/N: ").lower() == "y" else spammer() or "n" == sys.exit(0)
    else:
        print(f"")
        time.sleep(0)
        return spammer()
...
if __name__ == "__main__":
                ...
                if os.path.basename(sys.argv[0]).endswith("exe"):
                    search_for_updates()
                    if not os.path.exists(getTempDir()+"\\PhucAnhDepZai_proxies"):
                       clear()
                else:
                    search_for_updates()
                    if not os.path.exists(getTempDir()+"\\PhucAnhDepZai_proxies"):
